...
class AplRegistrationForm extends Component {
  ...
  handleUpload = (image) => {
    ...
    const uploadTask = storage.ref(`APL-2022/${this.state.email + '-' + image.name}`).put(image);
    uploadTask.on(
      "state_changed",
      snapshot => {
        ...
        >
          <div class="row register-form">
            ...
            <div style={{ margin: "15px auto 0 auto" }}>
              {this.props.loading?<Spinner/>:this.state.photo==""?<button
                type="submit"
                style={{
                  background: "#0DABF1",
                  color: "white",
           ...
           onClick={()=> ToastsStore.error("Complete the form to continue")}
              >
              Register
              </button>:<button
                type="submit"
